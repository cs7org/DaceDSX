...
void ProvisionImpl::scenario(ProvisionTopicMetadata& pmeta, datamodel::Scenario& scenario) {
    ...
    for (datamodel::BB s : sce->buildingBlocks) {
        std::cout << "\nInstanceID: " << s.instanceID << " SIMID: " << Config::getInstance()->getSimulatorID() << std::endl;
    ...
    for (datamodel::BB s : sce->buildingBlocks) {
        std::cout << "\n\n2. InstanceID: " << s.instanceID << " SIMID: " << Config::getInstance()->getSimulatorID() << std::endl;
...
void ProvisionImpl::waitForResources() {
    ...
    cout << "will wait for resources " << endl;
...
//            cout << rfm << endl;
            std::cout << rfm.path << " received: " << rfm.received << std::endl;
            if (!rfm.received) {
                cout << "waiting for '" << rfm.path << "' for: "<< WAITING_TIME << endl;
                usleep(WAITING_TIME);
                KDEBUG("waiting for '" << rfm.path << "'");
                stillwaiting = true;
            }
        }
        cout << "stillwaiting" << stillwaiting << endl;
    }
...
std::string ProvisionImpl::getFileByType(std::string type) {
    for (ResourceFileMeta rfm : resources) {
        std::cout<<"GETFILEBYTYPE: " << rfm.type << std::endl;
