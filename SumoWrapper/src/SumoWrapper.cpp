...
void daceDS::SumoWrapper::runWrapper() {
    ...
    std::shared_ptr<InteractionHandler> ihdl = std::make_shared<InteractionHandler>(producer); //interactionImpl, producer);
...
//    createDirs();
    ...
    std::cout << "looking for resource at " <<  Config::getInstance()->getResourceDir() << std::endl;
    ...
    for(std::string link : responsibleLinks){
        std::string topic = Config::getInstance()->getProvisionTopic("edge."+escapeSpecialChars(link)+".vehicles");
        std::cout<<"RESPONSIBLE LINK TOPIC : " <<topic <<std::endl;
    ...
    try {
        ...
        if (interactionConsumer){
            std::cout << "interactionConsumer is nullptr" << std::endl;
        }else {
            std::cout << "interactionConsumer is set" << std::endl;
