...
int PrepareRun::startSUMO(std::string sceID, std::shared_ptr<datamodel::BB> sim, int port, std::string config) {   
    ...
    if (pid == 0) {
        ...
        for (int c = 0; c < pam.size(); c++) {
            arr[c] = const_cast<char*>(pam[c].c_str());
        }
        arr[pam.size()] = nullptr; // Use nullptr instead of NULL in C++
...
//    std::cout << results <<std::endl;
    ...
    std::cout<<"RESULT ZIP AT: " << filename <<std::endl;
    std::string resDir = "";
    std::string fileList = "";
    if (returnMode == ReturnMode::ALL) {
        std::cout<<"RETURN MODE: ALL" <<std::endl;
        ...
        std::cout<<"RETURN MODE: RESOURCES" <<std::endl;
        //zip everything in basedir
        ...
        std::cout<<"RETURN MODE: LOGS" <<std::endl;
        //zip everything in basedir
        ...
        std::cout<<"RETURN MODE: LISTED" <<std::endl;
