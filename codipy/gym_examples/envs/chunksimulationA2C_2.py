...
class env_a2c_v2(gym.Env):
    ...
    """
    def __init__(self):
        self.config = '/home/niebisch/Documents/chunksimulation/config_seeding_strategy_update2.xml'
        ...
        rand_val = random.choice([50, 100, 150, 200])
        rand_val = 200
        self.update_size = 1000000 * rand_val  # random.choice(data_sizes)
        self.update_index = int(rand_val/50.0) - 1  # data_sizes.index(self.update_size)
        self.veh_density = int(6/2.0) - 1
        ...
        self.observation_space = spaces.MultiDiscrete([1001, 1001, 1001, 4, self.timesteps * (self.action_size - 1), 6])
...
#        os.chdir(self.output_abs_path)
    ...
    def step(self, action: ActType) -> Tuple[ObsType, float, bool, dict]:
        ...
        if self.timestep_counter > self.timesteps:
            ...
            return ([min(int(round((float(self.state)/float(self.max_chunks)), 3)*1000.0), 1000), min(int(round((float(self.total_seed)/float(self.max_chunks)), 3)*1000.0), 1000), 1000, min(self.update_index, 3), min(self.added_actions, 1199), min(self.veh_density, 5)], self.reward, done, self.info)

        remained = self.max_chunks - self.state
        self.added_actions += action
        ...
        print(done, self.max_chunks, self.state)
        print(done or self.max_chunks == self.state, done, self.max_chunks == self.state)
        if done or self.max_chunks == self.state:
           print("This says True")
        ...
        #print("done", type(done))
        #print("state", self.state)
        return ([min(int(round((float(self.state)/float(self.max_chunks)), 3)*1000.0), 1000), min(int(round((float(self.total_seed)/float(self.max_chunks)), 3)*1000.0), 1000), min(int(round((float(self.timestep_counter)/float(self.timesteps)), 3)*1000.0), 1000), min(self.update_index, 3), min(self.added_actions, 1199), min(self.veh_density, 5)], self.reward, done,  self.info)
...
#        print(self.vehicle_fleet.get_update_progress_fleet(), finished , round(self.traciStep, 4), round(float(self.duration_parameter), 4), 0)
#        print(self.vehicle_fleet.get_update_progress_fleet(), finished or round(self.traciStep, 4) == round(float(self.duration_parameter), 4), 0)
#        print(self.vehicle_fleet.get_update_progress_fleet(), finished)
...
#            print(self.vehicle_fleet.get_update_progress_fleet(), finished , round(self.traciStep, 4), round(float(self.duration_parameter), 4), 0)
    ...
    def reset(self):
        ...
        #print(self.info)
        return [0, 0, 0, min(self.update_index, 3), self.added_actions, min(self.veh_density, 5)]
