...
class env_v3(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.MultiDiscrete([1001, 1001, 1001])
    ...
    def step(self, action: ActType) -> Tuple[ObsType, float, bool, dict]:
        ...
        if self.timestep_counter > self.timesteps:
            ...
            return ([int(round((float(self.state)/float(self.max_chunks)), 3)*1000.0), int(round((float(self.total_seed)/float(self.max_chunks)), 3)*1000.0), 1000], self.reward, done, self.info)

        remained = self.max_chunks - self.state
        seeding_number = int(action * 0.01 * self.max_chunks)
        ...
        #print("done", type(done))
        #print("state", self.state)
        return ([int(round((float(self.state)/float(self.max_chunks)), 3)*1000.0), int(round((float(self.total_seed)/float(self.max_chunks)), 3)*1000.0), int(round((float(self.timestep_counter)/float(self.timesteps)), 3)*1000.0)], self.reward, done,  self.info)
    ...
    def reset(self):
        ...
        pathlib.Path(self.output_abs_path).mkdir(parents=True, exist_ok=True)
        os.chdir(self.output_abs_path)
        sumo_cmd = [self.sumo_binary, "-c", self.sumo_cfg, "--seed", str(int(self.initial_seeds)), "-S"]
