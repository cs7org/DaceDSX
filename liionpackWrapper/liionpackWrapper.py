...
from liionpackApi import liionpackAPI
...
energySchemaPath = this_directory+"/../AvroSchemas/Battery.avsc"
...
class liionpackWrapper():
    def __init__(self, scenarioID, instanceID):
        self.regexTopic = ["^provision\.simulation\." + scenarioID + "\.energy\.Battery"]
    ...
    def prepare(self):
        ...
        self.producer.create_topics([self.topicPre+"battery"])
    ...
    def run(self):
        ...
        try:
            ...
            self.api = liionpackAPI(self.timeSync, self.bbConsumer, self.producer, self.scenarioID, self.instanceID, stepLengthMin, self.sce['simulationEnd']/self.sim['stepLength'], to_observe=[], parameters=self.parameters)
            self.startMainConsumer()
            ...
            while self.timeSync.currentLocalTime < end:
                ...
                try:
                    power = self.api.prepareStep(iteration)
                except Exception as e:
...
def main():
    ...
    try:
        ctrl = liionpackWrapper(scenarioID ,instanceID)
